...
# load the iou files
with open(iou_output_path + 'original_store_giou.csv', 'r') as f:
    original = json.load(f)
with open(iou_output_path + 'updated_store_giou.csv', 'r') as f:
    updated = json.load(f)
with open(iou_output_path + 'giou.csv', 'r') as f:
    iou = json.load(f)
...
def compare():
    ...
    plt.title('GIOU - tv monitor confidence')
    ...
    plt.plot([giou_thresh for i in range(90)], 'k', label='IOU threshold')
